   ...
   const dispatch = useDispatch();
    ...
    <div className={s.centerblock__content}>  
            ...
            <div className={`${s['playlist-title__col']} ${s.col04}`}>
                ...
                <div className={`${s.content__playlist} ${s.playlist}`} onClick={(event) => setListTrack(event)} >  
                {
                    data.map((item,index) => <PlaylistItem track_file={item.track_file} id={item.id} track={item.name} author={item.author} album={item.album} time={(item.duration_in_seconds/60).toFixed(2)} key={index} stared_user={item.stared_user}/>)
